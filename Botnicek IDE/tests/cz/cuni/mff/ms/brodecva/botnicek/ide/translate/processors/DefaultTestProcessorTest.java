...
@PrepareForTest({ PatternArc.class, PredicateTestArc.class, CodeTestArc.class,
...
public class DefaultTestProcessorTest {
    ...
    private static final String MIXED_PATTERN =
            "MIXED PATTERN * WITH * WILDCARDS AND <bot name=\"bot\"/>";
    ...
    public void setUp() throws Exception {
        ...
        EasyMock.expect(this.targetStub.getName()).andStubReturn(
                NormalWords.of(TARGET_NAME));
        EasyMock.replay(this.targetStub);

        this.tested =
                DefaultTestProcessor.create(
                        this.settings.getTestingPredicate(),
                        this.settings.getSuccessState(),
                        this.settings.getReturnState());

        this.renderer =
                DefaultRenderingVisitor.create(this.settings
    ...
    public void testCreate() {
        DefaultTestProcessor
                .create(this.settings.getTestingPredicate(),
                        this.settings.getSuccessState(),
                        this.settings.getReturnState());
    }
    ...
    public void testProcessCodeTestArc() {
        ...
        final CodeTestArc codeTestArcStub =
                PowerMock.createMock(CodeTestArc.class);
        EasyMock.expect(codeTestArcStub.getCode()).andStubReturn(
                this.arcCodeStub);
        EasyMock.expect(codeTestArcStub.getName()).andStubReturn(
                NormalWords.of(CODE_TEST_ARC_NAME));
        EasyMock.expect(codeTestArcStub.getTested()).andStubReturn(
                testedCodeStub);
        EasyMock.expect(codeTestArcStub.getValue()).andStubReturn(
                Patterns.create(EXPECTED_VALUE_PATTERN));
        EasyMock.expect(codeTestArcStub.getTo()).andStubReturn(this.targetStub);
        PowerMock.replay(codeTestArcStub);

        final String testingPredicateName =
                this.settings.getTestingPredicate().getText();

        AbstractElement.acceptForEach(this.tested.process(codeTestArcStub),
                this.renderer);
        assertEquals("<topic name=\"" + CODE_TEST_ARC_NAME + " *\">"
                + "<category>" + "<pattern>*</pattern>" + "<that>*</that>"
                + "<template>" + "<think><set name=\"" + testingPredicateName
                + "\">" + TESTED_CODE + "</set></think>" + "<condition name=\""
                + testingPredicateName + "\">" + "<li value=\""
                + EXPECTED_VALUE_PATTERN + "\">" + ARC_CODE
                + "<think><set name=\"TOPIC\">" + TARGET_NAME
                + " <topicstar/></set></think>" + "</li>" + "<li>"
                + "<think><set name=\"TOPIC\"><topicstar/></set></think>"
                + "</li>" + "</condition>" + "<think><set name=\""
                + testingPredicateName + "\"></set></think>" + "<sr/>"
    ...
    public void testProcessPredicateTestArc() {
        ...
        final PredicateTestArc predicateTestArcStub =
                PowerMock.createMock(PredicateTestArc.class);
        EasyMock.expect(predicateTestArcStub.getCode()).andStubReturn(
                this.arcCodeStub);
        EasyMock.expect(predicateTestArcStub.getName()).andStubReturn(
                NormalWords.of(PREDICATE_TEST_ARC_NAME));
        EasyMock.expect(predicateTestArcStub.getPrepareCode()).andStubReturn(
                prepareCodeStub);
        EasyMock.expect(predicateTestArcStub.getPredicateName()).andStubReturn(
                NormalWords.of(TESTED_PREDICATE_NAME));
        EasyMock.expect(predicateTestArcStub.getValue()).andStubReturn(
                Patterns.create(EXPECTED_VALUE_PATTERN));
        EasyMock.expect(predicateTestArcStub.getTo()).andStubReturn(
                this.targetStub);
        PowerMock.replay(predicateTestArcStub);

        AbstractElement.acceptForEach(
                this.tested.process(predicateTestArcStub), this.renderer);
        assertEquals("<topic name=\"" + PREDICATE_TEST_ARC_NAME + " *\">"
                + "<category>" + "<pattern>*</pattern>" + "<that>*</that>"
                + "<template>" + PREPARE_CODE + "<condition name=\""
                + TESTED_PREDICATE_NAME + "\">" + "<li value=\""
                + EXPECTED_VALUE_PATTERN + "\">" + ARC_CODE
                + "<think><set name=\"TOPIC\">" + TARGET_NAME
                + " <topicstar/></set></think>" + "</li>" + "<li>"
                + "<think><set name=\"TOPIC\"><topicstar/></set></think>"
                + "</li>" + "</condition>" + "<sr/>" + "</template>"
    ...
    public void testProcessRecurentArc() {
        final EnterNode referredNodeStub =
                EasyMock.createStrictMock(EnterNode.class);
        EasyMock.expect(referredNodeStub.getName()).andStubReturn(
                NormalWords.of(REFERRED_NAME));
        EasyMock.replay(referredNodeStub);

        final RecurentArc recurentArcStub =
                PowerMock.createMock(RecurentArc.class);
        EasyMock.expect(recurentArcStub.getCode()).andStubReturn(
                this.arcCodeStub);
        EasyMock.expect(recurentArcStub.getName()).andStubReturn(
                NormalWords.of(RECURENT_ARC_NAME));
        EasyMock.expect(recurentArcStub.getTarget()).andStubReturn(
                referredNodeStub);
        EasyMock.expect(recurentArcStub.getTo()).andStubReturn(this.targetStub);
        PowerMock.replay(recurentArcStub);

        AbstractElement.acceptForEach(this.tested.process(recurentArcStub),
                this.renderer);
        assertEquals("<topic name=\"" + RECURENT_ARC_NAME + " *\">"
                + "<category>" + "<pattern>*</pattern>" + "<that>*</that>"
                + "<template>" + "<think><set name=\"TOPIC\">" + REFERRED_NAME
                + AIML.WORD_DELIMITER
                + this.settings.getReturnState().getText()
                + AIML.WORD_DELIMITER + RECURENT_ARC_NAME
                + " <topicstar/></set></think>" + "<sr/>" + "</template>"
                + "</category>" + "</topic>" + "<topic name=\""
                + this.settings.getSuccessState().getText()
                + AIML.WORD_DELIMITER + RECURENT_ARC_NAME + " *\">"
                + "<category>" + "<pattern>*</pattern>" + "<that>*</that>"
                + "<template>" + ARC_CODE + "<think><set name=\"TOPIC\">"
                + TARGET_NAME + " <topicstar/></set></think>" + "<sr/>"
                + "</template>" + "</category>" + "</topic>",
                this.renderer.getResult());
    ...
    public void testProcessTransitionArc() {
        final TransitionArc transitionArcStub =
                PowerMock.createMock(TransitionArc.class);
        EasyMock.expect(transitionArcStub.getCode()).andStubReturn(
                this.arcCodeStub);
        EasyMock.expect(transitionArcStub.getName()).andStubReturn(
                NormalWords.of(TRANSITION_ARC_NAME));
        EasyMock.expect(transitionArcStub.getTo()).andStubReturn(
                this.targetStub);
        PowerMock.replay(transitionArcStub);

        AbstractElement.acceptForEach(this.tested.process(transitionArcStub),
                this.renderer);
        assertEquals("<topic name=\"" + TRANSITION_ARC_NAME + " *\">"
                + "<category>" + "<pattern>*</pattern>" + "<that>*</that>"
                + "<template>" + ARC_CODE + "<think><set name=\"TOPIC\">"
                + TARGET_NAME + " <topicstar/></set></think>" + "<sr/>"
                + "</template>" + "</category>" + "</topic>",
