...
public class SegmentTest {
    ...
    public void testContainsWhenAroundEndAsArgumentReturnsTrue() {
        final Segment point =
                Segment.create(new Point(0, 0), new Point(100, 100));
    ...
    public void testContainsWhenEndAsArgumentReturnsTrue() {
        ...
        assertTrue(point.contains(coordinates.x, coordinates.y,
                DOUBLE_COMPARISONS_TOLERANCE));
    }
    ...
    public void testEqualsAndHashCode() {
        EqualsVerifier.forClass(Segment.class).usingGetClass()
    ...
    public void testGetLengthWhenNegativeAndPositiveCoordinatesReturnsSame() {
        final Segment segmentInPositive =
                Segment.create(new Point(3, 2), new Point(1, 1));
        final Segment segmentInNegative =
                Segment.create(new Point(-3, -2), new Point(-1, -1));
        assertEquals(segmentInPositive.getLength(),
                segmentInNegative.getLength(), DOUBLE_COMPARISONS_TOLERANCE);
    }
    ...
    public void testGetLengthWhenPointReturnsZero() {
        ...
        final Segment point = Segment.create(coordinates, coordinates);
        assertEquals(0, point.getLength(), 0); // Aritmetika typu zaručuju
                                               // exaktní práci s nulovými
                                               // argumenty.
    }
    ...
    public void testGetLengthWhenUnitDiagonalReturnsSquareRootOfTwo() {
        ...
        final Segment segment =
                Segment.create(new Point(0, 0), new Point(1, 1));
        assertEquals(Math.sqrt(2), segment.getLength(),
                DOUBLE_COMPARISONS_TOLERANCE);
    }
    ...
    public void testMoveFromAndGetFromDoesNotChangeTo() {
        ...
        final Segment original =
                Segment.create(new Point(2, 3), new Point(originalToX,
                        originalToY));
    ...
    public void testMoveFromAndGetFromReflectsNewFrom() {
        final Segment original =
                Segment.create(new Point(2, 3), new Point(-100, 2_000));
    ...
    public void testMoveToAndGetToDoesNotChangeFrom() {
        ...
        final Segment original =
                Segment.create(new Point(originalFromX, originalFromY),
                        new Point(2, 3));
    ...
    public void testOffset() {
        ...
        assertEquals(Segment.create(new Point(1, -1), new Point(3, 1)),
                original.offset(Math.sqrt(2)));
    }
    ...
    public void testOffsetAndGetLengthWhenSetOffReturnsEqualLength() {
        final Segment original =
                Segment.create(new Point(2, 3), new Point(-3, 1));
        final Segment offset = original.offset(5);

        assertEquals(offset.getLength(), original.getLength(),
                DOUBLE_COMPARISONS_TOLERANCE);
    }
    ...
    public void testOffsetWhenHorizontal() {
        final Segment original =
                Segment.create(new Point(2, 3), new Point(4, 3));

        assertEquals(Segment.create(new Point(2, 2), new Point(4, 2)),
                original.offset(1));
    }
    ...
    public void testToBounds() {
        final Rectangle source =
                new Rectangle(new Point(2, 3), new Dimension(2, 3));

        final Segment segment =
                Segment.create((int) source.getMinX(), (int) source.getMinY(),
                        (int) source.getMaxX(), (int) source.getMaxY());
