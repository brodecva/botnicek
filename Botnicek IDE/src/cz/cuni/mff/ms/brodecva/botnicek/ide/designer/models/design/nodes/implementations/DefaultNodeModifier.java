...
import cz.cuni.mff.ms.brodecva.botnicek.ide.designer.models.design.utils.TableUtils;
...
public class DefaultNodeModifier implements NodeModifier {
     ...
     */
    @Override
    public Node change(final Node node, final String name, final int x, final int y, final Class<? extends Node> type) {
        ...
        if (!node.getName().equals(name)) {
            this.namingAuthority.release(oldName);
            newName = this.namingAuthority.use(name);
        } else {
            newName = oldName;
        }
        ...
        try {
            factoryMethod = mappedClass.getMethod(NODE_FACTORY_METHOD_NAME, newName.getClass(), node.getParent().getClass(), Integer.TYPE, Integer.TYPE);
     ...
     */
    @Override
    public Node change(Node node, String name) {
