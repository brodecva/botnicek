...
/**
 * <p>
 * Pomocné metody pro práci s virtuálním zásobníkem, který je implementován
 * pomocí porovnání vzorů vůči tématu.
 * </p>
 * <p>
 * Odebrání vrcholu je realizováno pomocí porovnání vzoru "VRCHOL *" vůči
 * aktuálnímu tématu, a uložení části zachycené hvězdičkou jakožto nového tématu
 * </p>
 * <p>
 * Přidání vrcholu je realizováno uložením konkatenace nového vrcholu a
 * aktuálního tématu jakožto nového tématu.
 * </p>
 * <p>
 * Metody slouží pouze ke konstrukci modifikačního kódu a neověřují, zda-li
 * výstup parametrů vytvoří validní strukturu zásobníku v proměnné tématu.
 * </p>
 * 
...
public final class Stack {
    ...
    private static Joiner NORMAL_WORDS_JOINER = Joiner.on(AIML.WORD_DELIMITER
            .getValue());
     ...
     * 
     * @param first
     *            prvky, jejichž výstup bude přidán na začátek zásobníku
     * @param second
     *            prvky, jejichž výstup bude přidán za první, oddělené
     *            oddělovačem slov
    ...
    /**
     * Vytvoří téma, jež bude voláno při stavu zásobníku, ve kterém se nachází
     * složený název tématu na vrcholu.
     * 
     * @param composedName
     *            složený název tématu
     * @param categories
     *            kategorie tématu
     * @return téma stavu
     */
    public static Topic createState(final List<? extends NormalWord> composedName,
        ...
        return Topic.create(
                Patterns.create(Stack.joinWithSpaces(namesCopy)
                        + AIML.WORD_DELIMITER + AIML.STAR_WILDCARD),
                ImmutableList.copyOf(categories));
    }

    /**
     * Vytvoří téma, jež bude voláno při stavu zásobníku, ve kterém se nachází
     * název tématu na vrcholu.
     * 
     * @param name
     *            název tématu
     * @param categories
     ...
     * @return téma stavu
     */
    public static Topic createState(final NormalWord name,
            final Category... categories) {
        Preconditions.checkNotNull(name);
        Preconditions.checkNotNull(categories);

        return createState(ImmutableList.of(name),
                ImmutableList.copyOf(categories));
    }
    ...
    public static String joinWithSpaces(final List<? extends NormalWord> words) {
        ...
        return NORMAL_WORDS_JOINER.join(Collections2.transform(wordsCopy,
                new Function<NormalWord, String>() {
                    @Override
    ...
    /**
     * Vrátí strukturu, která odebere vrchol zásobníku, pokud je provedena.
     * Očekává, že zbytek zásobníku je zachycen žolíkem hvězdička.
     * 
    ...
    /**
     * Vrátí strukturu, která přidá výsledek zpracování prvků šablony na vrchol
     * zásobníku, pokud je provedena.
     * 
     * @param content
     *            prvky, jež se mají zpracovat a výsledek uložit na zásobník
    ...
    public static TemplateElement
        ...
        return concatenate(ImmutableList.copyOf(content),
                ImmutableList.<TemplateElement> of(Topicstar.create()));
    }

    /**
     * Vrátí strukturu, která přidá výsledek zpracování prvků šablony na
     * uvolněný vrchol zásobníku, pokud je provedena.
     * 
     * @param content
     *            prvky, jež se mají zpracovat a výsledek uložit na zásobník
     * @return strom prvků šablony, který provede přidání výsledku na zásobník
    ...
    /**
     * Vrátí strukturu, která přidá slova na uvolněný vrchol zásobníku, pokud je
     * provedena.
     * 
     * @param words
     *            normální slova
    ...
    public static TemplateElement prepend(final List<? extends NormalWord> words,
        ...
        if (contentCopy.isEmpty()) {
            ...
            final List<TemplateElement> pushedList =
                    ImmutableList.<TemplateElement> of(Text
                            .create(joinWithSpaces(contentCopy)));
    ...
    public static TemplateElement set(final List<? extends TemplateElement> content) {
        ...
        final Set update =
                Set.create(NormalWords.of(AIML.TOPIC_PREDICATE.getValue()),
