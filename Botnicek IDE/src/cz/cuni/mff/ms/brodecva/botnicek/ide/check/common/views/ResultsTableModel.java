...
/**
 * <p>
 * Model tabulky pro zobrazení seřazených výsledků neúspěšných kontrol zadaných
 * řetězců oproti datovým typům.
 * </p>
 * <p>
 * Umožňuje aktualizovat výsledky tak, že nahrazuje ty, které mají stejný
 * předmět. Výsledek úspěšné kontroly nebude přidán, pouze smaže související
 * negativní.
 * </p>
 * 
...
public final class ResultsTableModel extends AbstractTableModel {
    ...
    /**
     * Komparátor pro přehledné řazení výsledků (není slučitelný s ekvivalencí
     * výsledků).
     */
    private static final class SortingComparator implements
        ...
        public int compare(final CheckResult first, final CheckResult second) {
            ...
            final int sourceComparation =
                    first.getSource().toString().compareTo(second.toString());
            if (sourceComparation != 0) {
                return sourceComparation;
            }

            final int lineComparison =
                    Integer.compare(first.getErrorLineNumber(),
                            second.getErrorLineNumber());
            if (lineComparison != 0) {
                return lineComparison;
            }

            final int columnComparison =
                    Integer.compare(first.getErrorColumnNumber(),
                            second.getErrorColumnNumber());
            if (columnComparison != 0) {
                return columnComparison;
            }

            final int messageComparison =
                    first.getMessage().compareTo(second.getMessage());
    ...
    private static final Comparator<CheckResult> SORTING_COMPARATOR =
            new SortingComparator();
    private static final int COLUMNS_COUNT = 4;
    private static final List<String> COLUMNS_NAMES = ImmutableList.of(
            UiLocalizer.print("Description"), UiLocalizer.print("Source"),
            UiLocalizer.print("Line"), UiLocalizer.print("Column"));
    private static final List<Class<?>> COLUMNS_CLASSES = ImmutableList
            .<Class<?>> of(String.class, String.class, Integer.class,
                    Integer.class);
    ...
    /**
     * Seznam, který je udržován seřazený v pořadí: podle zdroje, čísla řádky,
     * sloupce a zprávy.
     */
    ...
    public Object getValueAt(final int rowIndex, final int columnIndex) {
        ...
        final List<Object> columns =
                Lists.<Object> newArrayList(row.getMessage(), row.getSource()
                        .toString(), row.getErrorLineNumber(), row
