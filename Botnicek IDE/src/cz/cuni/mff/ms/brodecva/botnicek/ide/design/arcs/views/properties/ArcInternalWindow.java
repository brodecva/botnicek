...
public class ArcInternalWindow extends ArcViewAdapter {
    ...
    private static final Map<Class<? extends Arc>, String> DEFAULT_SUPPORTED_TYPES_TO_DESCRIPTIONS = ImmutableMap.<Class<? extends Arc>, String>of(
                PatternArc.class, UiLocalizer.print("PatternTest"),
                PredicateTestArc.class, UiLocalizer.print("PredicateTest"),
                CodeTestArc.class, UiLocalizer.print("CodeTest"),
                TransitionArc.class, UiLocalizer.print("Transition"),
                RecurentArc.class, UiLocalizer.print("Recursion"));
    
    private static final List<Class<? extends Arc>> DEFAULT_SUPPORTED_TYPES =
            ImmutableList.<Class<? extends Arc>>of(TransitionArc.class, PatternArc.class, CodeTestArc.class, PredicateTestArc.class, RecurentArc.class);

    private static final int SAVE_BUTTON_WIDTH = 100;

    private static final int SAVE_BUTTON_HEIGHT = 24;

    private static final Dimension SAVE_BUTTON_DIMENSION = new Dimension(SAVE_BUTTON_WIDTH, SAVE_BUTTON_HEIGHT);
    ...
    private final JPanel typeChoicePanel = new JPanel(new WrapLayout(WrapLayout.CENTER));
    private final ButtonGroup typeChangeButtonGroup = new ButtonGroup();
    
    private Class<? extends Arc> currentType;
    private final Map<Class<? extends Arc>, JRadioButton> typesToRadios = new HashMap<>();
        ...
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    ...
                    final ArcInternalWindow arcInternalWindow = ArcInternalWindow.create(desktopPane);
                    final JScrollPane contentPane = new JScrollPane(desktopPane);
    ...
    private static ArcInternalWindow create(final Container parent) {
        return create(parent, DummyArcController.create(), DummyAvailableReferencesController.create(), DummyNormalWordValidationController.create(), DummyCodeValidationController.create(), DummySimplePatternValidationController.create(), DummyMixedPatternValidationController.create(), DummyNormalWordValidationController.create());
    }
     ...
     * @param availableReferencesController řadič dostupných uzlů k zanoření výpočtu
     * @param nameValidationController řadič validace názvu hrany
     * @param codeValidationController řadič validace provedeného kódu
     * @param simplePatternValidationController řadič validace vzoru očekávané hodnoty
     * @param mixedPatternValidationController řadič validace složených vzoru kategorií
     * @param predicateNameValidationController řadič validace názvu predikátu
     * @return správce vnitřního rámu
     */
    public static ArcInternalWindow create(final Container parent, final ArcController arcController, final AvailableReferencesController availableReferencesController, final NormalWordValidationController nameValidationController, final CodeValidationController codeValidationController, final SimplePatternValidationController simplePatternValidationController, final MixedPatternValidationController mixedPatternValidationController, final NormalWordValidationController predicateNameValidationController) {
        ...
        final MainPanel mainArcPanel = MainPanel.create(frame, arcController, predicateNameValidationController);
        final CodePanel codePanel = CodePanel.create(frame, arcController, codeValidationController);
        
        final CodeTestArcPanel codeTestArcPanel = CodeTestArcPanel.create(frame, arcController, codeValidationController, simplePatternValidationController);
        final PatternArcPanel patternArcPanel = PatternArcPanel.create(frame, arcController, mixedPatternValidationController);
        final PredicateTestArcPanel predicateTestArcPanel = PredicateTestArcPanel.create(frame, arcController, codeValidationController, simplePatternValidationController, predicateNameValidationController);
        final RecurentArcPanel recurentArcPanel = RecurentArcPanel.create(arcController, availableReferencesController);
        final TransitionArcPanel transitionArcPanel = TransitionArcPanel.create(arcController);
        
        final ImmutableMap.Builder<Class<? extends Arc>, AbstractTypePanel> arcTypesToElementsBuilder = ImmutableMap.builder();
        ...
        arcTypesToElementsBuilder.put(PredicateTestArc.class, predicateTestArcPanel);
        ...
        return create(parent, frame, mainArcPanel, codePanel, arcTypesToElementsBuilder.build(), DEFAULT_SUPPORTED_TYPES_TO_DESCRIPTIONS, DEFAULT_SUPPORTED_TYPES, arcController);
    }
     ...
     * 
     * @param parent rodičovský kontejner
     * @param frame samotný vnitřní rám
     * @param mainArcPanel panel s obecným nastavením hrany
     * @param codePanel panel s provedeným kódem v případě úspěšného testu
     * @param typesToPanels přiřazení typů hran k panelům tvořícím části rámu
     * @param typesToDescriptions přiřazení typů k jejich popiskům
     * @param supportedTypes podporované typy hran
     * @param arcController řadič vlastností hrany
     * @return správce rámu
     */
    public static ArcInternalWindow create(final Container parent, final ReferrableInternalFrame frame, final MainPanel mainArcPanel, final CodePanel codePanel, final Map<Class<? extends Arc>, AbstractTypePanel> typesToPanels, final Map<Class<? extends Arc>, String> typesToDescriptions, final List<Class<? extends Arc>> supportedTypes, final ArcController arcController) {
        ...
        final ArcInternalWindow newInstance = new ArcInternalWindow(frame, mainArcPanel, codePanel, typesToPanels, typesToDescriptions, supportedTypes, arcController);
        
        for (final Entry<Class<? extends Arc>, JRadioButton> provided : newInstance.typesToRadios.entrySet()) {
        ...
        newInstance.saveButton.addActionListener(new ActionListener() {
            ...
            @Override
            public void actionPerformed(final ActionEvent e) {
                final TypeView currentTypeElement = newInstance.arcTypesToElements.get(newInstance.currentType);
                
                currentTypeElement.save(mainArcPanel.getNewName(), mainArcPanel.getPriority(), codePanel.getCode());
            }
        });
    ...
    private ArcInternalWindow(
            final ReferrableInternalFrame frame,
            ...
            final List<Class<? extends Arc>> orderedSupportedTypes, final ArcController arcController) {
        Preconditions.checkNotNull(arcController);
        Preconditions.checkNotNull(frame);
        Preconditions.checkNotNull(mainArcPanel);
        Preconditions.checkNotNull(codePanel);
        ...
        Preconditions.checkArgument(typesToDescriptions.keySet().equals(typesToElements.keySet()));
        Preconditions.checkArgument(Sets.newHashSet(orderedSupportedTypes).equals(typesToDescriptions.keySet()));
        ...
        for (final Class<? extends Arc> type : orderedSupportedTypes) {
            ...
            final JRadioButton typeChangeRadio = new JRadioButton(description, false);
        ...
        this.typeChoicePanel.setBorder(BorderFactory.createTitledBorder(UiLocalizer.print("ArcTypeChange")));
        this.typesPanel.setBorder(BorderFactory.createTitledBorder(UiLocalizer.print("TransitionCondition")));
        this.codePanel.setBorder(BorderFactory.createTitledBorder(UiLocalizer.print("ExecutedCode")));
    ...
    /* (non-Javadoc)
     * @see cz.cuni.mff.ms.brodecva.botnicek.ide.design.arcs.views.types.ArcViewAdapter#updatedName(cz.cuni.mff.ms.brodecva.botnicek.ide.aiml.types.NormalWord)
     */
    @Override
    public void updateName(final NormalWord name) {
        ...
        this.frame.setTitle(name.getText() + VIEW_COPY_IDENTIFIER_SEPARATOR + Integer.toHexString(hashCode()));
    }
    ...
    private void clearAll() {
        final Collection<AbstractTypePanel> panels = this.arcTypesToElements.values();
    ...
    private void closeParts() {
        for (final TypeView typeElement : this.arcTypesToElements.values()) {
            typeElement.close();
        }
        
        this.mainArcPanel.close();
