 ...
 * @version 1.0
 */
public final class ArcUI extends FramedComponent {
    ...
    /**
     * Vytvoří komponentu hrany a zaregistruje na ní posluchače tak, aby po poklepání v jejím blízkém okolí byl zaslán požadavek řadič na zobrazení jejích vlastností.
     * 
     * @param arc model hrany
     * @param from reprezentace výchozího uzlu
     * @param to reprezentace cílového uzlu
     * @param arcsController řadič pro odstranění hrany
     * @param arcPropertiesController řadič pro zobrazení vlastností hrany
     * @return komponenta hrany v zobrazeném grafu
     */
    public static ArcUI create(final Arc arc, final NodeUI from, final NodeUI to, final ArcsController arcsController, final ArcPropertiesDisplayController arcPropertiesController) {
        return create(arc.getName(), arc.getPriority(), toType(arc), from, to, arcsController, arcPropertiesController);
    }
    
    private static ArcUI create(final NormalWord name, final Priority priority, final ArcType type, final NodeUI from, final NodeUI to, final ArcsController arcsController, final ArcPropertiesDisplayController arcPropertiesController) {
        return create(name, priority, type, from, to, Offset.NO_OFFSET, DEFAULT_THICKNESS, arcsController, arcPropertiesController);
    }
    
    private static ArcUI create(final NormalWord name, final Priority priority, final ArcType type, final NodeUI from, final NodeUI to, final Offset offset, final int thickness, final ArcsController arcsController, final ArcPropertiesDisplayController arcPropertiesController) {
        ...
        final ArcUI newInstance = new ArcUI(from, to, arcsController, arcPropertiesController);
    ...
    private boolean removalConfirmed() {
        final int removalConfirmed = JOptionPane.showConfirmDialog(this, UiLocalizer.print("ArcRemovalConfirmMessage"), UiLocalizer.print("ArcRemovalConfirmTitle"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    ...
    private ArcUI(final NodeUI from, final NodeUI to, final ArcsController arcsController, final ArcPropertiesDisplayController arcPropertiesController) {
    ...
    /**
     * Vrátí úsečku, která jako diagonála vyznačuje polohu a rozměry komponenty. Může se odchylovat v kolmém směru od nejkratší spojnice zobrazených uzlů v případě, že existuje hrana protisměrná.
     * 
     * @return úsečka hrany
     */
    ...
    private final void setPriority(Priority priority) {
        Preconditions.checkNotNull(priority);
        
        this.priority = priority;
    }

    private final void setType(ArcType type) {
        Preconditions.checkNotNull(type);
    ...
    public boolean contains(final int x, final int y) {
        final boolean result = getSegment().contains(x + getX(), y + getY(), this.thickness / 2d);
        return result;
    }
    ...
    public void paintComponent(final Graphics graphics) {
        ...
        graphics.drawString(String.format("%s (%s)", this.name.getText(), this.priority.getValue()), xCenter, yCenter);
    }
    ...
    private void drawArrowArms(final int xTip, final int yTip,
        ...
        graphics2d.rotate(-ARROW_ARM_ANGLE);
        final Line2D.Double rightSideLine = new Line2D.Double(0, 0, ARROW_SIDE_LENGTH, 0);
    ...
    /**
     * Oznámí komponentě, že se posunul jeden z jejích koncových uzlů, aby se mohla překreslit.
     */
    ...
    /**
     * Oznámí komponentě, že byla vůči ní přidána protisměrná hrana, aby se mohla překreslit.
     */
    ...
    /**
     * Oznámí komponentě, že byla odebrána její protisměrná hrana, aby se mohla překreslit.
     */
    public void jointContracted() {
        this.setOffset(Offset.NO_OFFSET);
        jointMoved();
    }
     ...
     * 
     * @param newVersion nová verze modelu hrany
     */
    ...
    public void arcRetyped(final Arc newVersion) {
        ...
        this.setType(toType(newVersion.getClass()));
    }
}
