...
import cz.cuni.mff.ms.brodecva.botnicek.ide.designer.models.aiml.elements.template.lists.DefaultListItem;
import cz.cuni.mff.ms.brodecva.botnicek.ide.designer.models.aiml.elements.template.lists.ListItem;
import cz.cuni.mff.ms.brodecva.botnicek.ide.designer.models.aiml.elements.template.lists.ValueOnlyListItem;
...
public final class SinglePredicateCondition extends AbstractProperElement implements ConditionElement {
    ...
    public static SinglePredicateCondition of(final PredicateName name, final ListItem... items) {
        return of(name, Arrays.asList(items));
    }
    
    public static SinglePredicateCondition of(final PredicateName name, final List<ListItem> items) {
    ...
    public static SinglePredicateCondition create(final PredicateName name, final DefaultListItem defaultItem, final ValueOnlyListItem... items) {
        Preconditions.checkNotNull(items);
        
        return create(name, defaultItem, Arrays.asList(items));
    }
    
    public static SinglePredicateCondition create(final PredicateName name, final ValueOnlyListItem... items) {
        Preconditions.checkNotNull(items);
        
        return create(name, Arrays.asList(items));
    }
    
    public static SinglePredicateCondition create(final PredicateName name, final DefaultListItem defaultItem, final List<ValueOnlyListItem> items) {
        Preconditions.checkNotNull(defaultItem);
        
        return new SinglePredicateCondition(name, Optional.of(defaultItem), items);
    }
    
    public static SinglePredicateCondition create(final PredicateName name, final List<ValueOnlyListItem> items) {
        return new SinglePredicateCondition(name, Optional.<DefaultListItem>absent(), items);
    }
    
    private SinglePredicateCondition(final PredicateName name, final Optional<DefaultListItem> defaultItem, final List<ValueOnlyListItem> items) {
