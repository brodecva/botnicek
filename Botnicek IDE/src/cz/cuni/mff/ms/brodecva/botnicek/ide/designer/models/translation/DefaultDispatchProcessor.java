...
public final class DefaultDispatchProcessor implements
    ...
    public static DefaultDispatchProcessor create(final String randomizeState, final List<TemplateElement> stack) {
        return new DefaultDispatchProcessor(randomizeState, stack);
    }
    
    private DefaultDispatchProcessor(final String randomizeState, final List<TemplateElement> stack) {
    ...
    public void process(final RandomNode node) {
        ...
        final Set enterRandomizerState = Set.create(PredicateNames.of(AIML.TOPIC.getValue()), Text.create(randomizeState));
    ...
    private static String concatenateMultiplied(final java.util.Set<Arc> targets) {
        ...
        for (final Arc target : targets) {
            ...
            for (int copyIndex = 0; copyIndex < repetitions; copyIndex++) {
                pushedStates.append(name + AIML.WORD_DELIMITER.getValue());
            }
        }
