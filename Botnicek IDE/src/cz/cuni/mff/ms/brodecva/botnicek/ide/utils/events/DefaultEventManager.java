...
/**
 * <p>
 * Výchozí implementace správce událostí.
 * </p>
 * <p>
 * Kromě toho, že realizuje odesílání zpráv o události tak, aby klient nemusel
 * rozlišovat, jaký typ odesílá, tak dovoluje v případě potřeby neodebírat
 * posluchač, aniž by docházelo k unikání paměti.
 * </p>
 * <p>
 * Přestože jsou instance této třídy serializovatelné, nelze toto garantovat pro
 * vložené objekty.
 * </p>
 * 
...
public final class DefaultEventManager implements EventManager, Serializable {
    ...
    /**
     * <p>
     * Návštěvník pro rozlišení typu události při vypravování posluchačům.
     * </p>
     * <p>
     * Díky němu je událost podle svého typu rozesílána správným posluchačům.
     * </p>
     * <p>
     * Při průchodu posluchači bere v potaz vlastnosti {@link WeakHashMap} při
     * iteraci přes členy.
     * </p>
     */
    private final class MappedEventVisitor implements Visitor {
        ...
        private <L> void visitCommon(final Event<L> event) {
            ...
            final Set<L> listeners =
                    (Set<L>) DefaultEventManager.this.eventsToListeners
                            .get(event.getClass());
        ...
        private <K, L> void visitMapped(final MappedEvent<K, L> event) {
            @SuppressWarnings("unchecked")
            final Map<K, Set<?>> keysTolisteners =
                    (Map<K, Set<?>>) DefaultEventManager.this.eventsAndKeysToListeners
                            .get(event.getClass());
            ...
            @SuppressWarnings("unchecked")
            final Set<L> mappedListeners =
                    (Set<L>) keysTolisteners.get(event.getKey());
            if (Presence.isAbsent(mappedListeners)) {
                return;
            }

            final Set<L> listenersSnapshot =
                    ImmutableSet.copyOf(mappedListeners);
            for (final L listener : listenersSnapshot) {
                event.dispatchTo(listener);
            }
        }
    ...
    private final Map<Class<? extends MappedEvent<?, ?>>, Map<?, Set<?>>> eventsAndKeysToListeners =
            new HashMap<>();

    private final Map<Class<? extends Event<?>>, Set<?>> eventsToListeners =
            new HashMap<>();
    ...
    public <K, L> void addListener(
        ...
        @SuppressWarnings("unchecked")
        final Map<K, Set<?>> keysTolisteners =
                (Map<K, Set<?>>) this.eventsAndKeysToListeners.get(type);
     ...
     */
    @Override
    public <K, L> void removeListener(
            final Class<? extends MappedEvent<K, L>> type, final K key,
            final L listener) {
        Preconditions.checkNotNull(type);
        Preconditions.checkNotNull(key);
