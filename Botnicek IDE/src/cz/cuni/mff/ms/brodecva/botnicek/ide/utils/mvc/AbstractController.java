...
/**
 * Pomocná třída, která poskytuje dědícím třídám základní prostředky pro správu pohledů a interakci s událostmi zasílanými modelem.
 * 
...
public abstract class AbstractController<V> implements Controller<V> {
    ...
    private final Table<Class<? extends MappedEvent<?, ?>>, Object, Set<?>> eventsAndKeysToStrongListenerRefs = HashBasedTable.create();
    private final SetMultimap<Class<? extends Event<?>>, ?> eventsToStrongListenerRefs = HashMultimap.create();
    
    /**
     * Postaví řadič na základě dodaného správce událostí, od kterého bude přijímat a do kterého bude zasílat zprávy o událostech.
     * 
     ...
     * 
     * @param callback instrukce k provedení na pohledu
     */
    protected final void callViews(final Callback<V> callback) {
        final Set<V> viewsSnapshot = ImmutableSet.copyOf(views);
        
        for (final V view : viewsSnapshot) {
            try {
                callback.call(view);
            } catch (final RuntimeException e) {
                throw e; //TODO: Pak zachytit.
            }
        }
    }
    
    /**
     * Zaregistruje posluchače pro daný klíč na správci událostí a u sebe podrží silnou referenci, která zamezí okamžitému zapomenutí správcem, který nedostupné posluchače maže.
     * 
    ...
    private <L> void addListenerStrongReference(final Class<? extends Event<L>> type, final L listener) {
        @SuppressWarnings("unchecked")
    ...
    private <L> void removeListenerStrongReference(final Class<? extends Event<L>> type, final L listener) {
        @SuppressWarnings("unchecked")
        final Set<L> listeners = (Set<L>) this.eventsToStrongListenerRefs.get(type);
    ...
    /**
     * Odebere posluchače pro daný klíč na správci událostí a u sebe smaže silnou referenci, která by jinak zamezila úklidu paměti.
     * 
     * @param type typ události
     * @param key klíč
     * @param listener posluchač
    ...
    /**
     * Přidá posluchače na správci událostí a u sebe podrží silnou referenci, která zamezí okamžitému zapomenutí správcem, který nedostupné posluchače maže.
     * 
     * @param type typ události
     * @param listener posluchač
     */
    ...
    /**
     * Odebere posluchače na správci událostí a u sebe smaže silnou referenci, která by jinak zamezila úklidu paměti.
     * 
     * @param type typ události
     * @param listener posluchač
     */
    ...
    /**
     * Provede odstranění posluchačů pro všechny posluchače registrované přes tento řadič.
     * 
     * @param type typ události
     * @param key klíč
     */
    protected final <K, L> void removeAllListeners(final Class<? extends MappedEvent<K, L>> type, final K key) {
        ...
        while (listenerIterator.hasNext()) {
            this.eventManager.removeListener(type, key, listenerIterator.next());
